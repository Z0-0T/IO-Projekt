<?php
	...
	if($_SERVER['REQUEST_METHOD'] == 'GET') {	
				...
				<html lang="pl-PL">
					...
					<body>
						...
						<form action="#" id="forma">
							 <table class="creds">
							  ...
							  <tr>
								<td colspan="2">
									...
									<?php for($i = 0; $i < count($role); $i++) { ?>
										<option value="<?php print($role[$i]->getRoleId()) ?>"><?php print($role[$i]->roleShortcutToFull()) ?></option>
									<?php } ?>
									</select>
								</td>
							  </tr>
							  ...
							  <tr>
								...
								<td>
									<button type="button" class="btn-green" id="send">Dodaj</button>
								</td>
							  </tr>
						...
						function createCredentials() {
							...
							let obj = {'imie': imie, 'nazwisko': nazwisko, 'telefon': telefon, 'dataUrodzenia': dataUrodzenia, 'kodPocztowy': kodPocztowy, 'miasto': miasto, 'adresZamieszkania': adresZamieszkania, 'adresEmail': adresEmail, 'haslo': haslo, 'reHaslo': reHaslo, 'roleId': roleId};
		...
		if($user != NULL) {
			$response["errors"] = 1;
			$response["emailError"] = "Użytkownik o takim adresie email już istnieje.";
			echo json_encode($response);
			return;
		} else {
			$user = User::createUserWithCredentials(NULL, $json["adresEmail"], $json["telefon"], $json["haslo"], $json["imie"], $json["nazwisko"], $json["dataUrodzenia"], $json["miasto"], $json["kodPocztowy"], $json["adresZamieszkania"], $json["roleId"]);

			$_SESSION["user"]->dodajPracownika($user);
